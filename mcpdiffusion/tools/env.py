...
SEARCH_DOCUMENTS = {
    ...
    "tool_description": (
        ...
        "(Insee Premiere, Insee Analyses, Dossiers, References, Chiffres-cles, "
        "Focus, ...). Returns structured publication records; pass the URL "
        "of a record to `get_insee_document` to fetch the full text.\n"
        "\n"
        "WHEN TO USE\n"
        "- Detailed tables or historic series.\n"
        "- Regional / departmental / municipal breakdowns.\n"
        "- Methodological notes, definitions, or full-text analysis.\n"
        "- Specific thematic studies (demography, labour market, public "
        "finances, environment, ...).\n"
        "\n"
        "WHEN NOT TO USE\n"
        "- Quick up-to-date indicator: use `get_insee_homepage`.\n"
        "- Rapid Releases (Informations rapides): use `search_insee_conjoncture`.\n"
        "- Concept definitions / code-list vocabulary: use `query_insee_rmes`.\n"
        "\n"
        "HOW TO SEARCH WELL\n"
        "- `query` -- natural-language query enriched with synonyms, related "
        "concepts and (when possible) a target year and/or geography.\n"
        "- `chiffre_clef=True` -- restrict to key-figure extracts "
        "(inflation, chomage, ...).\n"
        "- `geo_niveau` + `geo_keyword` -- territorial filtering. "
        "`geo_niveau` is one of COM / DEP / REG / INTER / COMPRD / FRANCE.\n"
        "- `theme` -- top-level INSEE theme (e.g. Demographie, "
        "Marche du travail - Salaires). Default is ALL.\n"
        "- `year_of_reference` -- hard filter on publication year; leave as "
        "null to search all years.\n"
        "\n"
        "OUTPUT\n"
        "A list of publications: `{ id, score, titre, soustitre, chapo, "
        "anneediffusion, zone, theme, url }`. `url` is ready to feed to "
        "`get_insee_document`.\n"
        f"\n"
